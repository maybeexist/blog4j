...
public class PostService implements IPostService
{
	...
	public int add(Post post, int[] tagIds)
	{
		...
		int id = DAO.execute(sql, post.getUrl(), post.getTitle(), post.getContent(), new Date(),
				post.getTop(), post.getStatus(), post.getType());
	...
	public int modify(Post post, int[] tagIds)
	{
		...
		int r = DAO.execute(sql, post.getUrl(), post.getTitle(), post.getContent(), post.getTop(),
				post.getStatus(), post.getType(), post.getId());
	...
	public List<Post> queryPublishRelativePost(Post post, int count)
	{
		...
		condition += " and id in (select post_id from bj_post_tag where tag_id in" + tagids
	...
	private List<Post> query(String condition, String order, PageInfo page, boolean calcPage)
	{
		...
		return populateTag(DAO.queryPage(POST_WRAPPER, sql, page.getCurrentIndex(),
				page.getPageSize()));
	}
	...
	private String keywordsCondition(String kw)
	{
		...
		for (String keywords : strs)
		{
			...
			condition.append("title like '%").append(keywords).append("%' or content like '%")
